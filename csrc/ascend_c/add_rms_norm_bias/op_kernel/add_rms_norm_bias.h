 ...
 *
 * AddRmsNormBias kernel: computes y = RMSNorm(x1 + x2) * gamma + beta
 * Normal mode – one row per iteration, full hidden dimension fits in UB.
 */

#ifndef ADD_RMS_NORM_BIAS_H_
#define ADD_RMS_NORM_BIAS_H_
#include "./rms_norm_base.h"

extern "C" {
#include "op_host/add_rms_norm_bias_tiling.h"
}
...
public:
    __aicore__ inline KernelAddRmsNormBias(TPipe* pipe) { Ppipe = pipe; }

    __aicore__ inline void Init(
        GM_ADDR x1, GM_ADDR x2, GM_ADDR gamma, GM_ADDR beta,
        GM_ADDR y, GM_ADDR rstd, GM_ADDR x,
        const AddRmsNormBiasTilingData* tiling)
    {
    ...
    __aicore__ inline void SubProcess(
        uint32_t i_o, uint32_t calc_row_num,
...
private:
    ...
    __aicore__ inline void Compute(
        ...
        LocalTensor<float> gammaLocal, LocalTensor<float> betaLocal,
    ...
    __aicore__ inline void Compute(
        uint32_t inner_progress,
        LocalTensor<half> gammaLocal, LocalTensor<half> betaLocal,
        LocalTensor<float> rstdLocal)
    {
    ...
    __aicore__ inline void Compute(
        uint32_t inner_progress,
        LocalTensor<bfloat16_t> gammaLocal, LocalTensor<bfloat16_t> betaLocal,
        LocalTensor<float> rstdLocal)
    {
        LocalTensor<float> x_fp32 = xFp32Buf.Get<float>();
        LocalTensor<float> sqx = sqxBuf.Get<float>();
